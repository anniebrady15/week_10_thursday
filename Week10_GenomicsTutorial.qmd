...
F statistic close to 1 - not good evidence for a relationship between rs12225351 and height.

p value is small so not significant.

Estimate = -0.14, so this SNP increases height by 0.14.

```{r}
effectsize(height_rs12225351_lm)
```

CI includes 0, not significantly significant.
...
```{r}
height_sumstats_chr20 <- height_sumstats_chr20 %>% 
  mutate(logP = -log10(P))
height_sumstats_chr20 %>% 
  ggplot(aes(x = POS, y = logP)) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  geom_hline(yintercept = 5e-8)
```
...
Position = chr20:35431781 (GRCh38.p14).
...
height_sumstats_chr11 <- height_sumstats_chr11 %>% 
  mutate(logP = -log10(P))
...
height_sumstats_chr11 %>%
  arrange(P) %>%
  slice_head(n=1)
#chr11:113334452 (GRCh38.p14), TTC12 tetratricopeptide repeat domain 12
...
mdd_gwas_chr11 <- snps_chr11 %>%
  map(~lm(pheno_chr11[['MDD']] ~ pheno_chr11[[.x]])) %>%
  #map(~summary(.x)$coefficients[2,]) %>%
  map(~summary(.x)) %>%
  map(~coefficients(.x)[2,]) %>%
  bind_rows() %>%
...
mdd_sumstats_chr11 <- mdd_sumstats_chr11 %>% 
  mutate(logP = -log10(P))
...
mdd_sumstats_chr11 %>%
  arrange(P) %>%
  slice_head(n=1)
#chr11:113500036 DRD2 dopamine receptor D2
```

```{r}
# fit a linear model for each SNP as the predictor and height as the outcome
# extract the second row of coefficients (for the SNP effect) from each model
mdd_gwas_chr20 <- snps_chr20 %>%
  map(~lm(pheno_chr20[['MDD']] ~ pheno_chr20[[.x]])) %>%
  #map(~summary(.x)$coefficients[2,]) %>%
  map(~summary(.x)) %>%
  map(~coefficients(.x)[2,]) %>%
  bind_rows() %>%
...
mdd_sumstats_chr20 <-
freq_chr20 %>%
  inner_join(mdd_gwas_chr20, by='ID') %>%
  select(CHR=`#CHROM`, SNP=ID, POS, REF, ALT, REF_FREQ,
         BETA=Estimate, SE=`Std. Error`, t=`t value`, P=`Pr(>|t|)`)

mdd_sumstats_chr20 <- mdd_sumstats_chr20 %>% 
  mutate(logP = -log10(P))
...
mdd_sumstats_chr20 %>%
  arrange(P) %>%
  slice_head(n=1)
#chr20:35497699 CEP250 	centrosomal protein 250
```
